...
func testUploadOneToOneUploadQueryUsingUploadQuery(t *testing.T) {

	tx := MustTx(boil.Begin())
	defer func() { _ = tx.Rollback() }()

	var foreign UploadQuery
	var local Upload

	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &foreign, uploadQueryDBTypes, true, uploadQueryColumnsWithDefault...); err != nil {
	...
	check, err := local.UploadQuery().One(tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadUploadQuery(tx, false, (*[]*Upload)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.UploadQuery == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.UploadQuery = nil
	if err = local.L.LoadUploadQuery(tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.UploadQuery == nil {
		t.Error("struct should have been eager loaded")
...
func testUploadOneToOneSetOpUploadQueryUsingUploadQuery(t *testing.T) {
	var err error
	...
	if err = randomize.Struct(seed, &b, uploadQueryDBTypes, false, strmangle.SetComplement(uploadQueryPrimaryKeyColumns, uploadQueryColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, uploadQueryDBTypes, false, strmangle.SetComplement(uploadQueryPrimaryKeyColumns, uploadQueryColumnsWithoutDefault)...); err != nil {
		t.Fatal(err)
	}
	...
	for i, x := range []*UploadQuery{&b, &c} {
		err = a.SetUploadQuery(tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.UploadQuery != x {
		...
		if exists, err := UploadQueryExists(tx, x.UploadID); err != nil {
			t.Fatal(err)
		} else if !exists {
...
var (
	uploadDBTypes = map[string]string{`ID`: `text`, `UserID`: `integer`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`, `Status`: `enum.upload_status('created','uploading','uploaded','query_sent','query_failed','completed','terminated','abandoned')`, `Size`: `bigint`, `Received`: `bigint`}
	_             = bytes.MinRead
)
