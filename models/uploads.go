...
var (
	uploadAllColumns            = []string{"id", "user_id", "created_at", "updated_at", "status", "size", "received"}
	uploadColumnsWithoutDefault = []string{"id", "user_id", "updated_at", "status", "size"}
...
// UploadQuery pointed to by the foreign key.
func (o *Upload) UploadQuery(mods ...qm.QueryMod) uploadQueryQuery {
	queryMods := []qm.QueryMod{
	...
	query := UploadQueries(queryMods...)
	queries.SetFrom(query.Query, "\"upload_queries\"")
...
// LoadUploadQuery allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-1 relationship.
func (uploadL) LoadUploadQuery(e boil.Executor, singular bool, maybeUpload interface{}, mods queries.Applicator) error {
	var slice []*Upload
	var object *Upload
	...
	query := NewQuery(qm.From(`upload_queries`), qm.WhereIn(`upload_queries.upload_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	results, err := query.Query(e)
	if err != nil {
		return errors.Wrap(err, "failed to eager load UploadQuery")
	}

	var resultSlice []*UploadQuery
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice UploadQuery")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results of eager load for upload_queries")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for upload_queries")
	}
	...
	if singular {
		foreign := resultSlice[0]
		object.R.UploadQuery = foreign
		if foreign.R == nil {
			foreign.R = &uploadQueryR{}
		}
...
// SetUploadQueryG of the upload to the related item.
// Sets o.R.UploadQuery to related.
// Adds o to related.R.Upload.
// Uses the global database handle.
func (o *Upload) SetUploadQueryG(insert bool, related *UploadQuery) error {
	return o.SetUploadQuery(boil.GetDB(), insert, related)
}

// SetUploadQuery of the upload to the related item.
// Sets o.R.UploadQuery to related.
// Adds o to related.R.Upload.
func (o *Upload) SetUploadQuery(exec boil.Executor, insert bool, related *UploadQuery) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.UploadQuery = related
	}

	if related.R == nil {
		related.R = &uploadQueryR{
			Upload: o,
		}
	} else {
		related.R.Upload = o
