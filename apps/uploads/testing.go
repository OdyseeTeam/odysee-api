package uploads

import (
	"context"
	"database/sql"
	"fmt"
	"os"
	"path"
	"testing"

	"github.com/OdyseeTeam/odysee-api/apps/uploads/database"
	"github.com/OdyseeTeam/odysee-api/pkg/bus"
	"github.com/OdyseeTeam/odysee-api/pkg/configng"
	"github.com/OdyseeTeam/odysee-api/pkg/migrator"
	"github.com/Pallinder/go-randomdata"

	"github.com/aws/aws-sdk-go/aws"
	"github.com/aws/aws-sdk-go/aws/awserr"
	"github.com/aws/aws-sdk-go/service/s3"
)

type UploadTestHelper struct {
	S3Config configng.S3Config
	DB       *sql.DB
	S3       *s3.S3
	Queries  *database.Queries
}

func NewUploadTestHelper(t *testing.T) (*UploadTestHelper, error) {
	th := &UploadTestHelper{}
	s3cfg := configng.S3Config{
		Endpoint: "http://localhost:9000",
		Bucket:   fmt.Sprintf("test-uploads-%s-%s", randomdata.Noun(), randomdata.Adjective()),
		Region:   "us-east-1",
		Key:      "minio",
		Secret:   "minio123",
		Minio:    true,
	}
	th.S3Config = s3cfg

	client, err := configng.NewS3Client(s3cfg)
	if err != nil {
		return nil, err
	}

	_, err = client.CreateBucket(&s3.CreateBucketInput{
		Bucket: aws.String(s3cfg.Bucket),
	})
	if err != nil {
		if awsErr, ok := err.(awserr.Error); !ok || awsErr.Code() != "BucketAlreadyOwnedByYou" {
			return nil, err
		}
	}

	t.Cleanup(func() {
		listObjectsOutput, err := client.ListObjects(&s3.ListObjectsInput{
			Bucket: aws.String(s3cfg.Bucket),
		})
		if err != nil {
			t.Logf("failed to list objects in bucket: %v", err)
			return
		}

		for _, object := range listObjectsOutput.Contents {
			t.Logf("deleting %s", *object.Key)
			_, err = client.DeleteObject(&s3.DeleteObjectInput{
				Bucket: aws.String(s3cfg.Bucket),
				Key:    object.Key,
			})
			if err != nil {
				t.Logf("failed to delete object: %v", err)
			}
		}

		_, err = client.DeleteBucket(&s3.DeleteBucketInput{
			Bucket: aws.String(s3cfg.Bucket),
		})
		if err != nil {
			t.Logf("failed to delete bucket: %v", err)
		}
	})

	db, cleanup, err := migrator.CreateTestDB(migrator.DefaultDBConfig(), database.MigrationsFS)
	if err != nil {
		return nil, err
	}
	t.Cleanup(func() { cleanup() })

	th.DB = db
	th.S3 = client
	th.Queries = database.New(db)
	return th, nil
}

func (th *UploadTestHelper) CreateUpload(filePath string, bus *bus.Client) (*database.Upload, error) {
	// This simulates IDs generated by TUS backend.
	uploadID := randomdata.RandStringRunes(32) + "+" + randomdata.RandStringRunes(32)
	uploadKey := randomdata.RandStringRunes(32)
	s, err := th.uploadFileToS3(filePath, uploadKey)
	if err != nil {
		return nil, err
	}
	up, err := th.Queries.CreateUpload(context.Background(), database.CreateUploadParams{
		UserID: int32(randomdata.Number(1, 10000)),
		ID:     uploadID,
		Size:   s,
	})
	completeUpload(th.Queries, bus, up.UserID, up.ID, path.Base(filePath), th.S3Config.Bucket, uploadKey)
	return &up, err
}

func (th *UploadTestHelper) uploadFileToS3(filePath, key string) (int64, error) {
	file, err := os.Open(filePath)
	if err != nil {
		return 0, err
	}
	defer file.Close()

	fileInfo, err := file.Stat()
	if err != nil {
		return 0, err
	}
	fileSize := fileInfo.Size()

	fmt.Sprintf("uploading %s to %s/%s\n", filePath, th.S3Config.Bucket, key)
	_, err = th.S3.PutObject(&s3.PutObjectInput{
		Bucket: aws.String(th.S3Config.Bucket),
		Key:    aws.String(key),
		Body:   file,
	})
	if err != nil {
		return fileSize, err
	}
	return fileSize, nil
}
