...
func (r *Router) GetAll() []*models.LbrynetServer {
	r.reloadServersFromDB()
	logger.WithField("lock", "mu").Trace("waiting for read lock in GetAll")
	r.mu.RLock()
	logger.WithField("lock", "mu").Trace("got read lock in GetAll")
	defer r.mu.RUnlock()
	return r.servers
}
...
func (r *Router) RandomServer() *models.LbrynetServer {
	r.reloadServersFromDB()
	logger.WithField("lock", "mu").Trace("waiting for read lock in RandomServer")
	r.mu.RLock()
	logger.WithField("lock", "mu").Trace("got read lock in RandomServer")
	defer r.mu.RUnlock()
...
func (r *Router) setServers(servers []*models.LbrynetServer) {
	...
	logger.WithField("lock", "mu").Trace("waiting for write lock in setServers")
	r.mu.Lock()
	logger.WithField("lock", "mu").Trace("got write lock in setServers")
	defer r.mu.Unlock()
	r.servers = servers
...
func (r *Router) updateLoadAndMetrics() {
	for _, server := range r.GetAll() {
		...
		if err != nil {
			...
			logger.WithField("lock", "loadMu").Trace("waiting for write lock in updateLoadAndMetrics 1")
			r.loadMu.Lock()
			logger.WithField("lock", "loadMu").Trace("got write lock in updateLoadAndMetrics 1")
			delete(r.load, server)
			r.loadMu.Unlock()
			metric.Set(-1.0)
			// TODO: maybe mark this instance as unresponsive so new users are assigned to other instances
		} else {
			logger.WithField("lock", "loadMu").Trace("waiting for write lock in updateLoadAndMetrics 2")
			r.loadMu.Lock()
			logger.WithField("lock", "loadMu").Trace("got write lock in updateLoadAndMetrics 2")
...
func (r *Router) LeastLoaded() *models.LbrynetServer {
	...
	logger.WithField("lock", "loadMu").Trace("waiting for read lock in LeastLoaded")
	r.loadMu.RLock()
	logger.WithField("lock", "loadMu").Trace("got read lock in LeastLoaded")
